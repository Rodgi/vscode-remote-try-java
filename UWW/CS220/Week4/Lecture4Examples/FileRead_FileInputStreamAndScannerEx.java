...
public class FileRead_FileInputStreamAndScannerEx {
	public static void main(String[] args) throws IOException { // add a throws statement here to throw IOException to
		...
		// common good practice: use relative path instead of absolute path to indicate
		// the location of a file
		// In Eclipse, the default data file location (without specifying file path) is
		// the project folder, e.g. Week-5
		// (1) what if we store that text file in src folder?
		fileByteStream = new FileInputStream("Lecture4Examples/myfile.txt"); // will look for the text file in Week-5 folder
		...
		while (inFS.hasNextInt()) { // use Scanner object methods such as hasNext(), hasNextLine(), hasNextInt() to
									// find next data in the file
			// the hasNextInt() method will return true if there are more data in the file,
